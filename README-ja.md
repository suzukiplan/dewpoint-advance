...
Windows の場合、テストビルドのタイミングでも package.conf のアイコンファイルが参照できなければビルドが通らないため、適当なアイコンファイル（256x256のpng）へのパスを指定してください。

> macOS ではパッケージ作成のタイミングで初めて IconFile が必要になり、Linux では IconFile を参照しません。
...
- Esc: Selectボタン
- Space: Startボタン
- macOS: ⌘+R でリセット、⌘+P でハードウェアポーズ、⌘+Q で電源OFF
...
| `dpa_ugc_limit_size_set` | 圧縮前 UGC データの上限サイズを設定 |
...
Leaderboard の再送用ファイルは Steam Auto-Cloud の対象には含まれません。OS が提供するユーザー別の
ローカルアプリケーションデータ領域に保存されます。
...
`dpa_leaderboard_send` で送信されたスコアはボードごとにローカルへ保存されます。Steamへの送信が完了する前に
ネットワーク接続が失われたりアプリケーションが終了した場合、次回起動時に各ボードにつき1回再送されます。
UGC付きスコアはUGCのLeaderboard添付まで完了した時点で処理済みになります。

- 再送データは `board0.dat` ～ `board15.dat` としてアプリ固有の `leaderboard-cache` ディレクトリへ保存されます。
- ファイルは同じOSユーザー内だけで利用され、Steam Auto-Cloudでは同期されません。
- Steamアカウントを特定できたデータはそのアカウントにだけ再送されます。
- Steam上に同一またはより良いスコアが存在する場合、そのデータは処理済みになります。
- 同一ボードへ再度送信すると、未送信データは新しいスコアとUGCで置き換えられます。ゲーム側ではローカルの
  ハイスコアが更新された時に呼び出してください。

再送ファイルはリトルエンディアンで、先頭12バイトが処理済みフラグ（未処理 `0x00000000`、処理済み
`0xFFFFFFFF`）、符号付き32bitスコア、UGCサイズ、続いてUGCデータ、
...
UGC のハードリミットは圧縮前の生データに適用されます。初期値は 1 MiB（`1048576` bytes）で、GBA
アプリケーションから `dpa_ugc_limit_size_set` で変更でき、`dpa_ugc_limit_size_get` で現在値を取得できます。
指定可能な値は 4 以上 2146435072 以下の 4 の倍数です。setter は実際の上限値を返し、不正な値を指定した場合や
UGC の処理中は値を変更しません。値を変更すると共通 UGC バッファはクリアされるため、UGC の追記、ダウンロード、送信より
前に設定してください。この上限値は共通バッファ、再送ファイル、Steam へのアップロード、ダウンロードおよび展開時の
検証へ一貫して適用されます。設定値はプロセスの終了または別の値を設定するまで有効です。独自の値を使用する
アプリケーションは起動時に設定してください。現在の上限を超える再送ファイルは破損扱いで隔離せず保留され、
アプリケーションが十分な上限値を設定した時点で読み込まれます。

上限値を増やすと、ローカルストレージ、メモリ、Steam Cloud の使用量も増える可能性があります。Steam Cloud の
クォータも適切に設定し、Runtime を移植する場合は対象とする全プラットフォームで利用可能な値を選択してください。
